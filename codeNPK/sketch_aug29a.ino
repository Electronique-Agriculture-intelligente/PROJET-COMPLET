...
*
* Créé le 26/08/2023
* Modifié tout le moi du 08/2024
*/
...
void handleReceivedSMS(void* pvParameters) {
  Serial.println("module sms activer !");
  for (;;) {
...
const char* ssid = "Laisse ces choses";   // Nom du point d'accès WiFi
const char* password = "jenepartagepas";  // Mot de passe du point d'accès WiFi
...
void setup() {
...
#elif defined(ESP32)
  SerialMon.begin(115200);  // c'etait a 115200
  modbus.begin(9600);
  pinMode(RE, OUTPUT);  // Configuration de la broche RE en sortie
  pinMode(DE, OUTPUT);  // Configuration de la broche DE en sortie
  ...
  SerialAT.begin(115200, SERIAL_8N1, MODEM_RX, MODEM_TX);  // c'etait a 115200
  delay(3000);
...
int httpPOSTRequest(String url) {
  ...
  http.begin(client, url);  // Initialisation de la communication HTTP vers le serveur
